...
#  [7] "Ebit"              "Employees"        
#  [9] "FilingRate"        "GDP"              
# [11] "HeadCityPop"       "HeadCourtCityToDE"
# [13] "HeadStAtFiling"    "Liab"             
# [15] "MonthFiled"        "PrimeFiling"      
# [17] "Sales"             "SIC"              
# [19] "SICMajGroup"       "YearFiled"   
...
nooutlier<- bank_data_op1%>%
    mutate(abbreviation=abbreviate(Name))%>%
  select(c(abbreviation,Assets,CPI,Ebit,Liab,FilingRate,GDP,PrimeFiling,Sales))%>%
  filter(abbreviation!="TIn.")
...
rownames(pca_no$x)<-pull(nooutlier,abbreviation)

autoplot(pca_no,label = TRUE, label.size = 3,
         loadings = TRUE, loadings.colour = 'blue',
         loadings.label = TRUE, loadings.label.size = 3)
...
Employees + Ebit + Liab + Assests + Sales with CPI + Filing + GDP

Employees + Ebit + Liab + Assests + Sales HAS Weak positive correlation with Days In + Prime filing
Employees + Ebit + Liab + Assests + Sales HAS Weak negative correlation with headcity pop and headcourtcitytoDE
...
```{r}
library(corrplot)
M = cor(data_clean%>%select_if(is.numeric))
...
PC1_boot <- boot(data = data_clean%>%select_if(is.numeric), compute_PC1, R = 1000)
colnames(PC1_boot$t) <- colnames(data_clean%>%select_if(is.numeric))
