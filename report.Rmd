...
data_clean %>% ungroup() %>% select_if(is.numeric) %>%
  scale %>% dist() %>% hclust(method = "complete") -> DataClusterC

DataClusterC %>% plot(cex=.5)
DataClusterC %>% rect.hclust(k=10)
```
...
```{r}
DataClusterK <- kmeans((data_clean %>% ungroup() %>% select_if(is.numeric)), 10, nstart = 5)
