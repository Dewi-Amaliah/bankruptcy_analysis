...
Ward D2 method gives the best clustering outcome with reasonable sized clusters so we compare all other clusters with this method.
...
We can observe that there is a big range of Height tolerance on y-axis for k = 3 with the next values at 34 and and 23 for k = 2 and k = 4 respectively. Thus, we cluster the data into three clusters as it has the highest range which keeps our clustering stable.  
...
```{r ave, fig.cap="Dendogram with average method"}
...
```{r cmp, fig.cap="Dendogram with complete method"}
...
```{r cnt, fig.cap="Dendogram with centroid method"}
